...
from .build_components import TRACK_EMBEDDING_UPDATER_REGISTRY

from .torch_utils import bbox_cxcywh_to_xyxy
from .torch_utils import inverse_sigmoid
from detectron2.structures import Instances, Boxes
from ..modeling.structures import pairwise_iou
...
@TRACK_EMBEDDING_UPDATER_REGISTRY.register()
class QueryInteractionModule(nn.Module):
    def __init__(self, dim_in, hidden_dim, random_drop=0.1, fp_ratio=0.3, update_query_pos=False, merger_dropout=0):
        super().__init__()
    ...
    def _add_fp_tracks(self, track_instances: Instances, active_track_instances: Instances) -> Instances:
            ...
            if len(inactive_instances) > 0 and len(selected_active_track_instances) > 0:
                ...
                if num_fp >= len(inactive_instances):
                    ...
                    inactive_boxes = Boxes(bbox_cxcywh_to_xyxy(inactive_instances.pred_boxes))
                    selected_active_boxes = Boxes(bbox_cxcywh_to_xyxy(selected_active_track_instances.pred_boxes))
    ...
    def _select_active_tracks(self, data) -> Instances:
        track_instances: Instances = data[0]
        if self.training:
    ...
    def _update_track_embedding(self, track_instances: Instances) -> Instances:
        ...
        out_embed = track_instances.output_embs
...
def build(cfg):
    ...
    layer_name = cfg.MODEL.TRACK_EMBEDDING_UPDATER
    dim_in = cfg.MODEL.QueryInteractionModule.dim_in
    hidden_dim = cfg.MODEL.QueryInteractionModule.hidden_dim
    random_drop = cfg.MODEL.QueryInteractionModule.random_drop
    fp_ratio = cfg.MODEL.QueryInteractionModule.fp_ratio
    update_query_pos = cfg.MODEL.QueryInteractionModule.update_query_pos
    merger_dropout = cfg.MODEL.QueryInteractionModule.merger_dropout
    assert layer_name in interaction_layers, 'invalid query interaction layer: {}'.format(layer_name)
    return interaction_layers[layer_name](dim_in, hidden_dim, random_drop, fp_ratio, update_query_pos, merger_dropout)